...
public class StaticFunction {
	...
	public static void sendNoticePublic(DiscordClient client) {
		client.getChannelById(StaticID.PUBLIC)
			.cast(TextChannel.class)
			.flatMap(txt -> 
				txt.createMessage(nMsg -> {
					nMsg.setEmbed(embed -> {
								embed.setColor(Color.RED);
								embed.setTitle("**__Utilisation du salon #public__**");
								embed.setDescription("Ce salon a pour but de mettre en relation des individus de manière publique, \n"
										+ "tout en gardant son anonymat. \n\n"
										+ "Vous pouvez envoyer des messages ce canal grâce à la commande ``!public <MESSAGE>``, "
										+ "vos messages seront postés à travers le bot et vous serez uniquement identifiable grâce à votre id ``abc1``");
							});
				})
			).subscribe();
	}
	...
	public static void updateInformation(DiscordClient client) {
		...
		if(channel.getLastMessageId().isPresent()) {
			channel.getMessagesBefore(channel.getLastMessageId().get())
				...
				.filter(msg -> msg != null)
				.subscribe(msg -> msg.delete().subscribe());
		}
		
		channel.createEmbed(embed -> {
				embed.setColor(Color.RED);
				embed.setTitle(":desktop: **__Informations HRP__** :desktop:");
		...
		channel.createEmbed(embed -> {
			embed.setColor(Color.RED);
			embed.setTitle("**__Informations sur discord (simplifié)__**");
			
			embed.setDescription("\n"
					+ "**__Les canaux__**\n"
					+ "Les canaux dans la catégorie **SALON PRIVE**, "
					+ "seul vous, le bot pouvez voir dans ce canal et discuter à travers votre ID.\n"
					+ "Utilisez donc le bot !msg 0000 Votre message\n"
					+ "Soyez malin et utilisez !report <Votre Message> si vous ne comprenez rien\n"
					+ "\n"
					+ "\n"
					+ "**__Les notifications__**\n"
					+ "Les seules notifications (mentions) de ce discord sont vis à vis de modifications importantes "
					+ "et informations très importantes, nous n'abuserons pas des notifications et elles seront limités à "
					+ "1 notification tout les 3 jours maximum.\n"
					+ "\n"
					+ "Vous pouvez tout à fait désactivé les notifications normales et/ou mentions si vous pensez que "
					+ "ce discord n'est que secondaire par rapport à votre RP.\n"
					+ "\n"
					+ "\n"
					+ "**__Radio crypté ?__**\n"
					+ "Nous avons prévu de mettre en place des salons vocaux à travers le bot dans le futur mais ceux_ci "
					+ "ne sont pas encore disponible pour le moment...\n");
	}).subscribe();
		
		Message msg = channel.createMessage(spec -> {
			spec.setEmbed(embed -> {
				embed.setColor(Color.RED);
				embed.setTitle(":desktop: **__Informations RP__** :desktop:");
				...
				embed.setDescription("\n"
						...
						+ "profiter d'échange crypter et intraçable !\n\n"
						+ "\n"
